...
namespace Zenject
{
    ...
    public class DiContainer : IInstantiator
    {
            ...
            IEnumerable<DiContainer> parentContainersEnumerable, bool isValidating)
        {
            ...
            {
                for (int i = 0; i < parentContainers.Length; i++) parentContainers[i].FlushBindings();
        ...
        private object CreateLazyBinding(InjectContext context)
        {
            ...
            if (IsValidating) QueueForValidate((IValidatable)result);
        ...
        {
            List<BindingId> rootBindings = new();
            List<ProviderInfo> rootProviders = new();
        ...
        private void FlushValidationQueue()
        {
            ...
            List<IValidatable> validatables = new();
            ...
            while (_validationQueue.Any())
            {
                ...
                for (int i = 0; i < validatables.Count; i++) validatables[i].Validate();
            }
        }
        ...
        {
            ProviderInfo info = new(provider, condition, nonLazy, this);
        ...
        private ProviderInfo TryGetUniqueProvider(InjectContext context)
        {
            ...
            for (int i = 0; i < containerLookups.Length; i++) containerLookups[i].FlushBindings();
        ...
        {
            List<DiContainer> processed = new();

            Queue<DiContainer> containerQueue = new();
            ...
            BindingId bindingId, InjectSources sourceType, List<ProviderInfo> buffer)
        {
            ...
            for (int i = 0; i < containerLookups.Length; i++) containerLookups[i].GetLocalProviders(bindingId, buffer);
        }
                ...
                try
                {
                    ...
                    try
                    {
                        for (int i = 0; i < matches.Count; i++)
                        {
                            ...
                            for (int k = 0; k < instances.Count; k++) allInstances.Add(instances[k]);
                        }
        ...
        public DecoratorToChoiceFromBinder<TContract> Decorate<TContract>()
        {
            ...
            FactoryBindInfo factoryBindInfo = new(
...
#if !UNITY_EDITOR
            if (prefabWasActive) prefabAsGameObject.SetActive(false);
#else
            if (prefabWasActive)
            {
...
#if !UNITY_EDITOR
            if (prefabWasActive) prefabAsGameObject.SetActive(true);
#else
            if (prefabWasActive)
            {
            ...
            GameObjectCreationParameters gameObjectBindInfo, InjectContext context)
        {
            ...
            GameObject gameObj = new(gameObjectBindInfo.Name ?? "GameObject");
            Transform parent = GetTransformGroup(gameObjectBindInfo, context);
        ...
        {
            GameObject gameObj = new(groupName);
            gameObj.transform.SetParent(ContextTransform, false);
            gameObj.transform.SetParent(null, false);
            ...
            IEnumerable<object> extraArgs)
        {
            ...
            GameObjectCreationParameters creationParameters = new()
            {
                ParentTransform = parentTransform,
                Position = position,
                Rotation = rotation
        ...
        public void InjectGameObject(GameObject gameObject)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < monoBehaviours.Count; i++) Inject(monoBehaviours[i]);
            }
            finally
            {
        ...
        public bool UnbindId(Type contractType, object identifier)
        {
            ...
            BindingId bindingId = new(contractType, identifier);
        ...
        {
            foreach (Type i in concreteType.Interfaces()) Unbind(i, concreteType);
        }
        ...
        {
            ConventionBindInfo conventionBindInfo = new();
            ...
            where TFactoryContract : IFactory
        {
            ...
            FactoryBindInfo factoryBindInfo = new(typeof(TFactoryConcrete));
        ...
        private MemoryPoolIdInitialSizeMaxSizeBinder<TItemContract> BindMemoryPoolCustomInterfaceInternal<TItemContract,
            ...
            where TPoolContract : IMemoryPool
        {
            List<Type> contractTypes = new() { typeof(IDisposable), typeof(TPoolContract) };
            ...
            FactoryBindInfo factoryBindInfo = new(typeof(TPoolConcrete));
            MemoryPoolBindInfo poolBindInfo = new();
            ...
            Type scriptableObjectType, string resourcePath, List<TypeValuePair> extraArgs)
        {
            ...
            Object newObj = ScriptableObject.Instantiate(objects.Single());
